...
def write_baseline(folder, bundle, tasks, target="raw"):
    ...
    for spec in runner.core.procedure_specs():
        if (spec["transform"] == "standard" and spec["penalty_grid"] == "original"
                and spec["pca_grid"] in ("none", "original")):
            old_names.append(runner.legacy_core.model_name(spec, target))
...
class LinearDesignRunnerTests(unittest.TestCase):
    def test_planned_matrix_is_complete_and_uses_numeric_history_sentinel(self):
        ...
        self.assertEqual(len({model["model_id"] for model in models}), 172)
        for target in ("raw", "dgtw"):
            self.assertEqual(sum(model["target"] == target for model in models), 86)
        for model in models:
    ...
    def test_baseline_provenance_and_direct_prediction_comparison(self):
        with directory() as folder:
            ...
            self.assertEqual(len(runner.verify_baseline_predictions(result, record, bundle)), 19)
    ...
    def test_publication_complete_coverage_and_saved_metadata(self):
        with directory() as folder:
            ...
            self.assertEqual(len(published["models"]), 86)
            ...
            self.assertEqual(len(selections), 86)
