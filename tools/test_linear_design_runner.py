...
import linear_design_runner as runner
...
def directory():
    ...
    folder = root / f"linear_design_runner_test_{uuid.uuid4().hex}"
    folder.mkdir(parents=True)
    try:
        yield folder
    finally:
        assert folder.resolve().parent == root.resolve()
        assert folder.name.startswith("linear_design_runner_test_")
...
def write_baseline(folder, bundle, tasks, target="raw"):
    ...
    for spec in runner.core.procedure_specs():
        if (spec["history"] == "fixed504" and spec["refit"] == "retain"
                and spec["representation"] == "full" and spec["estimator"] != "group_ridge"):
            old_names.append(runner.linear.model_name(spec["estimator"], spec["feature_set"], target, 504))
    ...
    path = folder / "checkpoints" / f"fit504_{target}_{tasks[0]['month']}.pkl"
    atomic_pickle(dict(fingerprint="baseline-fixture", task=tasks[0], result=old_result), path)
    config = dict(data_manifest=bundle["manifest"], prepared_manifest_sha256=runner.sha256_file(Path(bundle["path"]) / "manifest.json"),
                  validation_days=126, horizon=1, grid=runner.linear.grid_specification(), code=previous.code_identity(),
                  versions=runner.versions(), tasks=tasks)
    registry = dict(schema_version="characteristics_v1_linear", prepared=bundle["path"],
...
class LinearDesignRunnerTests(unittest.TestCase):
    def test_planned_matrix_is_complete_and_uses_numeric_history_sentinel(self):
        ...
        self.assertEqual(len({model["model_id"] for model in models}), 228)
        for target in ("raw", "dgtw"):
            self.assertEqual(sum(model["target"] == target for model in models), 114)
        for model in models:
    ...
    def test_baseline_provenance_and_direct_prediction_comparison(self):
        with directory() as folder:
            ...
            self.assertEqual(len(runner.verify_baseline_predictions(result, record, bundle)), 12)
    ...
    def test_publication_complete_coverage_and_saved_metadata(self):
        with directory() as folder:
            ...
            self.assertEqual(len(published["models"]), 114)
            ...
            self.assertEqual(len(selections), 114)
