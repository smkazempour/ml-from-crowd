"""Run the declared linear-optimization pilot, fitting, verification and reports."""
...
import linear_optimization_runner as runner
from linear_optimization_evaluate import family_counts


ROOT = Path(__file__).resolve().parents[1]
SOURCES = (
    "run_linear_optimization_study.py", "linear_optimization_core.py", "linear_optimization_runner.py",
    "linear_optimization_evaluate.py", "linear_optimization_report.py",
...
def validate_specification(spec):
    ...
    if (spec["total_models"] != 2 * len(core.procedure_specs()) or spec["test_start"] != "2014-01"
            ...
            or spec["experiment_id"] != "linear_optimization_v3"
...
def start_watcher(run, status):
    ...
    command = [sys.executable, "-B", str(ROOT/"tools/watch_study.py"),
               "--status", str(status), "--report", str(ROOT/"reports/LINEAR_OPTIMIZATION_COMPLETION.md"),
               "--label", "Linear optimization study", "--interval", "30"]
...
def main(argv=None):
    ...
    parser.add_argument("--spec", type=Path, default=ROOT/"reports/data/linear_optimization_v3_experiment.json")
    ...
    parser.add_argument("--out-root", type=Path, default=ROOT/".runs/linear_optimization_v3/study")
    ...
    with FileLock(str(run/"study.lock"), timeout=0):
        ...
        try:
            ...
            common = [ROOT/"tools/linear_optimization_runner.py", "--prepared", args.prepared,
            ...
            run_phase(base+[ROOT/"tools/linear_optimization_evaluate.py", "--registry", registry_path,
            ...
            run_phase(base+[ROOT/"tools/linear_optimization_report.py", "--registry", registry_path,
