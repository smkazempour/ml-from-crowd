"""Resumable factorial study of characteristic ranks, penalties and text PCA.

This module owns execution and provenance. The isolated linear_optimization_core
...
try:
    from . import linear_optimization_core as core, protocol_data as data, protocol_linear as linear
...
def code_identity():
    names = ("linear_optimization_runner.py", "linear_optimization_core.py",
             "linear_optimization_evaluate.py", "linear_design_runner.py", "linear_design_core.py", "protocol_data.py",
    ...
    for target in targets:
        for spec in core.procedure_specs():
            models.append(dict(model_id=core.model_name(spec, target), target=target,
                ...
                transform=spec["transform"], penalty_grid=spec["penalty_grid"], pca_grid=spec["pca_grid"],
...
def verify_baseline(bundle, registry_path, tasks_by_month, targets):
    ...
    if (registry.get("schema_version") != "linear_design_v2"
        ...
        raise ValueError("Historical linear-design configuration differs")
    ...
    for tasks in tasks_by_month:
        ...
        for target in targets:
            ...
            for base in legacy_core.base_specs():
                spec = {**base, "history": "fixed504", "refit": "union"}
                if legacy_core.model_name(spec, target) not in result["model_names"]:
...
def verify_baseline_predictions(result, baseline_record, bundle):
    ...
    for spec in core.procedure_specs():
        if (spec["transform"] != "standard" or spec["penalty_grid"] != "original"
                or spec["pca_grid"] not in ("original", "none")):
            continue
        name = core.model_name(spec, result["target"])
        old_name = legacy_core.model_name(spec, result["target"])
    ...
    if len(audit) != 19:
        raise ValueError("The baseline audit must compare all nineteen original procedures")
    return audit
