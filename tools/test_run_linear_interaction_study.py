...
import run_linear_interaction_study as study


class ControllerTests(unittest.TestCase):
    def setUp(self):
        self.base = Path(__file__).resolve().parents[1]/".runs"
        self.root = self.base/f"linear_interaction_controller_test_{uuid.uuid4().hex}"
        self.root.mkdir(parents=True)

        def cleanup():
            assert self.root.resolve().parent == self.base.resolve()
            assert self.root.name.startswith("linear_interaction_controller_test_")
    ...
    def test_specification_matches_actual_menu(self):
        path = Path(__file__).resolve().parents[1]/"reports/data/linear_interactions_v1_experiment.json"
