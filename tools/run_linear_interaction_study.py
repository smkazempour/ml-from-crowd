"""Run the declared linear-interaction pilot, fitting, verification and reports."""
...
SOURCES = (
    "run_linear_interaction_study.py", "linear_interaction_core.py", "linear_interaction_runner.py",
    "linear_interaction_evaluate.py", "linear_interaction_report.py",
    "linear_optimization_core.py", "linear_optimization_evaluate.py",
...
def validate_specification(spec):
    ...
    if (spec["total_models"] != 2 * len(core.procedure_specs()) or spec["test_start"] != "2014-01"
            or spec["test_end"] != "2022-12" or spec["test_months"] != 108
            or spec["validation_days"] != 126 or spec["targets"] != ["raw", "dgtw"]
            or spec["grid"] != core.grid_specification()
            or spec["procedures"] != core.procedure_specs()
            or spec["planned_family_counts"] != family_counts(runner.planned_models())
            or spec["experiment_id"] != "linear_interactions_v1"
            or spec["fixed_fit_days"] != 504 or spec["refit"] != "union"):
...
def start_watcher(run, status):
    """Confirm this attempt is monitored; retry an old watcher's transient lock."""
    command = [sys.executable, "-B", str(ROOT/"tools/watch_study.py"),
               "--status", str(status), "--report", str(ROOT/"reports/LINEAR_INTERACTION_COMPLETION.md"),
               "--label", "Linear interaction study", "--interval", "30"]
    current = json.loads(status.read_text(encoding="utf-8"))
    identity = {key: current[key] for key in ("fingerprint", "started_at", "pid")}
    deadline, process = time.monotonic() + 90, None
    while time.monotonic() < deadline:
        if process is None:
            with (run/"watcher.log").open("a", encoding="utf-8") as log:
        ...
        if code is not None:
            if code != 3:
                raise RuntimeError(f"Completion watcher exited {code}; see {run/'watcher.log'}")
            process = None
        else:
            try:
                health = json.loads((run/"watcher_status.json").read_text(encoding="utf-8"))
                observed = health.get("monitored_identity") or {}
                if (all(observed.get(key) == value for key, value in identity.items())
                        and health.get("watcher_pid") == process.pid and health.get("terminal") is None):
                    return process
    ...
    raise RuntimeError("Completion watcher could not acquire this attempt within 90 seconds; see watcher.log")


def main(argv=None):
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--spec", type=Path, default=ROOT/"reports/data/linear_interactions_v1_experiment.json")
    ...
    parser.add_argument("--baseline-registry", type=Path,
                        default=ROOT/".runs/linear_design_v2/study/c11478141261534f/linear/full_9590a962af3e2fcb/registry.json")
    parser.add_argument("--out-root", type=Path, default=ROOT/".runs/linear_interactions_v1/study")
    parser.add_argument("--workers", type=int, default=4)
    parser.add_argument("--threads", type=int, default=2)
    parser.add_argument("--no-notify", action="store_true", help="Disable the independent local completion/error watcher")
    ...
    with FileLock(str(run/"study.lock"), timeout=0):
        ...
        try:
            if not args.no_notify:
                watcher = start_watcher(run, status)
                update_status(status, state, watcher_pid=watcher.pid)
            validate_prepared(args.prepared)
            common = [ROOT/"tools/linear_interaction_runner.py", "--prepared", args.prepared,
            ...
            pilot_registry = output_path(pilot_log, "registry")
            update_status(status, state, pilot_registry=str(pilot_registry))
            pilot_prefix = run/"pilot_checks"/"evaluation"
            pilot_prefix.parent.mkdir(exist_ok=True)
            run_phase(base+[ROOT/"tools/linear_interaction_evaluate.py", "--registry", pilot_registry,
                      "--prepared", args.prepared, "--out", pilot_prefix, "--threads", args.threads,
                      "--allow-pilot", "--skip-subgroups", "--skip-horizons"],
                      run/f"pilot_evaluation_attempt_{attempt:03}.log", "pilot_evaluate", status, state, environment)
            run_phase(base+[ROOT/"tools/linear_interaction_report.py", "--registry", pilot_registry,
                      "--evaluation", pilot_prefix, "--out", run/"pilot_checks"/"report.md",
                      "--summary-output", run/"pilot_checks"/"summary.md", "--allow-pilot"],
                      run/f"pilot_report_attempt_{attempt:03}.log", "pilot_report", status, state, environment)
            if code_identity() != identity["code"] or sha256(args.spec) != identity["spec_sha256"]:
                raise ValueError("Study sources or specification changed during the pilot")
            ...
            summary = ROOT/spec["summary_report"]
            update_status(status, state, evaluation=str(prefix), report=str(report))
            run_phase(base+[ROOT/"tools/linear_interaction_evaluate.py", "--registry", registry_path,
            ...
            run_phase(base+[ROOT/"tools/linear_interaction_report.py", "--registry", registry_path,
                      "--evaluation", prefix, "--out", report, "--summary-output", summary], run/f"report_attempt_{attempt:03}.log",
            ...
            if not summary.is_file() or not summary.stat().st_size:
                raise ValueError("Missing readable summary report")
            artifacts.append(summary)
            update_status(status, state, status="complete", phase="complete", finished_at=timestamp(),
                          models=len(runner.planned_models()), summary_report=str(summary),
