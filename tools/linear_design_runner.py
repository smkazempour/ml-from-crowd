"""Resumable matched study of linear refitting, grouped penalties and text PCA.

This module owns execution and provenance. The isolated linear_design_core
...
try:
    from . import linear_design_core as core, protocol_data as data, protocol_linear as linear
...
def code_identity():
    names = ("linear_design_runner.py", "linear_design_core.py", "protocol_data.py",
...
def planned_models(targets=("raw", "dgtw")):
    ...
    for target in targets:
        for spec in core.procedure_specs():
            models.append(dict(model_id=core.model_name(spec, target), target=target,
                target_column=linear.TARGET_COLUMNS[target], estimator=spec["estimator"],
                feature_set=spec["feature_set"], representation=spec["representation"],
...
def validate_tasks(tasks):
    if len(tasks) != 3 or [task["fit_days"] for task in tasks] != list(FIT_DAYS):
        raise ValueError("Each monthly job requires the fixed ordered 504/252/756 tasks")
...
def verify_baseline(bundle, registry_path, tasks_by_month, targets):
    ...
    old = registry["config"]
    if (registry.get("schema_version") != "characteristics_v1_linear"
            ...
            or old.get("grid") != linear.grid_specification()):
        raise ValueError("Historical characteristic configuration differs")
    ...
    old_tasks = {(task["month"], task["fit_days"]): task for task in old["tasks"]}
    records = {}
    for tasks in tasks_by_month:
        ...
        task = tasks[0]
        for target in targets:
            path = registry_path.parent / "checkpoints" / f"fit504_{target}_{task['month']}.pkl"
            with path.open("rb") as source:
                saved = pickle.load(source)
            result = saved["result"]
            predictions = np.asarray(result["predictions"])
            if (saved["fingerprint"] != registry["fingerprint"] or saved["task"] != task
                    ...
                    or hashlib.sha256(predictions.tobytes()).hexdigest() != result.get("prediction_sha256")
                    or not np.isfinite(predictions).all()):
                raise ValueError(f"Historical checkpoint is invalid: {path}")
            for feature in FEATURE_SETS:
                for estimator in linear.ESTIMATORS:
                    if linear.model_name(estimator, feature, target, 504) not in result["model_names"]:
...
def verify_baseline_predictions(result, baseline_record, bundle):
    ...
    for spec in core.procedure_specs():
        if (spec["history"] != "fixed504" or spec["refit"] != "retain"
                or spec["representation"] != "full" or spec["estimator"] == "group_ridge"):
            continue
        name = core.model_name(spec, result["target"])
        old_name = linear.model_name(spec["estimator"], spec["feature_set"], result["target"], 504)
    ...
    if len(audit) != 12:
        raise ValueError("The baseline audit must compare all twelve retained models")
    return audit
...
def validate_month(result, tasks, target, bundle):
    ...
    if (result["month"] != tasks[0]["month"] or result["target"] != target
        ...
        raise ValueError(f"Invalid linear-design checkpoint: {target}/{tasks[0]['month']}")
    ...
    with FileLock(str(path) + ".lock", timeout=0):
        if path.exists():
            ...
            if (saved["fingerprint"] != run_fingerprint or saved["tasks"] != tasks
                ...
                raise ValueError(f"Linear-design checkpoint identity mismatch: {path}")
            validate_month(saved["result"], tasks, target, bundle)
            if result_certificate(saved["result"]) != saved.get("result_certificate"):
                raise ValueError(f"Linear-design checkpoint checksum mismatch: {path}")
...
def publish_run(bundle, run_dir, config, checkpoint_paths):
    ...
    if seen != set(expected_tasks):
        raise ValueError("Cannot publish an incomplete linear-design run")
    if code_identity() != config["code"]:
        raise ValueError("Linear-design fitting source changed during the run")
...
def main(argv=None):
    ...
    grid = core.grid_specification() if hasattr(core, "grid_specification") else dict(
        global_grid=linear.grid_specification(), pca_dimensions=[16, 32, 64, 128],
        social_multipliers=[1, 10], text_multipliers=[1, 10, 100], include_smaller_model=True)
